...
class DashboardRoute extends Component {
  ...
  //loads language data for user
  componentDidMount() {
    LanguagesService.getLanguage().then((res) =>
      this.setState({ language: res })
    );
  }

  render() {
    //redirects to login if token has expired
    if (
      this.state.language &&
      this.state.language.error === 'Unauthorized request'
    ) {
      ...
      return (
        <section aria-live="polite" className="dashboard-container">
          <div className="dashboard-upper">
            ...
            <p>
              Total correct answers: {this.state.language.language.total_score}
            </p>
            <p>
              <Link to="/learn">
